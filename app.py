...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/keyariawHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs', db.Column('gif_id', db.Integer, db.ForeignKey('Gif.id')), db.Column('search_terms_id', db.Integer, db.ForeignKey('SearchTerm.id')))
...
class User(UserMixin, db.Model):
    ...
    gifcoll = db.relationship('PersonalGifCollection', backref='User')
...
class Gif(db.Model):
    ...
    def __repr__(self):
        return "{} (title: {})".format(self.title,self.embedURL)
...
def get_gifs_from_giphy(search_string):
    ...
    param = {'api_key': api_key, 'q': search_string, 'limit': 5}
    search = json.loads(requests.get(url=baseurl, params=param).text)
...
def get_or_create_search_term(term):
    ...
    # It should iterate over that list acquired from Giphy and invoke get_or_create_gif for each, and then append the return value from get_or_create_gif to the search term's associated gifs (remember, many-to-many relationship between search terms and gifs, allowing you to do this!).
        for i in lis['data']:
            ...
            i = get_or_create_gif(i['title'],i['embed_url'] )
           # print(i)
            tm.gifs.append(i)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    print(current_user)
    print('HEREEEEEE')
    ##(User.query.filter_by(username=current_user).first()).id
    collection = db.session.query(PersonalGifCollection).filter_by(name=name, user_id=current_user.id).first()
    if collection:
        return collection
    else:
        collection = PersonalGifCollection(name=name,user_id=current_user.id,gifs=[])#might need to add current user
        for i in gif_list:

           # ey = get_or_create_gif(i,'')
            collection.gifs.append(i)
...
def create_collection():
    ...
    gifs = Gif.query.all()
    choice = [(g.id, g.title) for g in gifs]
    form.gif_picks.choices = choice

    if request.method == 'POST':
   
        lis_got =form.gif_picks.data
        col_picks = [get_gif_by_id(int(id)) for id in lis_got]
        collect = get_or_create_collection(form.name.data, current_user, col_picks)
        return redirect(url_for('collections', collect=collect))
...
def collections():
    ...
    collection = db.session.query(PersonalGifCollection).filter_by(user_id=current_user.id)
